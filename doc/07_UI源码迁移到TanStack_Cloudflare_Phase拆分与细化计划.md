...
- [ ] **Step 4: 配置 `vite.config.ts` 为 Cloudflare SSR 入口**
...
- [ ] **Step 6: 重写 `src/routes/__root.tsx`**
  - 移除 `next/font/google`
  - 移除 `Metadata`
...
- [ ] **Step 1: 迁移首页到 `src/routes/index.tsx`**
  - 先保留视觉布局和双卡入口
  - 状态区先使用静态占位，不提前接 D1

- [ ] **Step 2: 迁移营业额录入页到 `src/routes/sales/new.tsx`**
  - 先保留原型里的字段与布局
  - `useState` 暂时允许存在，等 Phase 7 再切 TanStack Form

- [ ] **Step 3: 迁移月分析页到 `src/routes/analytics/monthly.tsx`**
  - 只迁视觉层和 mock 卡片
  - 不在本阶段接 SQL 聚合

- [ ] **Step 4: 迁移日历页到 `src/routes/analytics/calendar.tsx`**
  - 先保留当前交互外观
  - 随机数据逻辑保留到 Phase 7 再清理

- [ ] **Step 5: 新建 `src/routes/calendar.tsx` 兼容旧 URL**
  - 只做 redirect，不承载业务 UI

- [ ] **Step 6: 改写 sidebar 导航**
...
- [ ] **Step 2: 建立 `src/routes/invoices/new.tsx`**
  - 页面只负责选择文件、上传文件、创建 intake job
  - 不在此页承载 review 表格和复杂编辑状态

- [ ] **Step 3: 建立 `src/routes/invoices/review/$jobId.tsx`**
